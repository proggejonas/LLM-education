...
function PromptAntiPatterns() {
  const patterns = [
    { prompt: "Make it better", why: "The AI equivalent of telling a chef to 'make the food more food'", icon: "\uD83E\uDD37" },
    { prompt: "Write something about AI", why: "For whom? A 5-year-old? The board? Your cat?", icon: "\uD83C\uDF0A" },
    { prompt: "Fix this", why: "[pastes 500 lines of code] 'It does not work.' Thanks, that narrows it down.", icon: "\uD83D\uDD25" },
    ...
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap", justifyContent: "center" }}>
        <div style={{ flex: "1 1 240px", maxWidth: 270 }}>
          ...
          {[
            { q: "How much is 12\u00d77?", why: "You have a calculator on your phone" },
            { q: "Draw a cat driving a car", why: "Entertaining, but not exactly ROI" },
            { q: "Tell me a fun fact", why: "It is not a party trick" },
            { q: "What is the capital of France?", why: "Congrats, you just built a slow search engine" },
          ].map((s, i) => (
...
const slides = [
  // ── TITLE ──
  {
    ...
    content: () => (
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ fontSize: "1.15rem", color: C.muted, maxWidth: 520, margin: "0 auto" }}>
          What it actually is, why it makes things up,<br/>and how to get it to stop
        </p>
      </div>
    ),
    notes: "Welcome everyone. Today I am going to demystify AI. And yes, before you ask, this presentation was partly made with AI. I figured if I am going to tell you how it works, I should probably prove that it does. By the end you will have a real mental model of what these tools are doing, what they are terrible at, and most importantly how to get much better results from them.",
  },
  ...
  { section: "01 \u00b7 What Is an LLM?", color: C.blue,
    ...
    notes: "This is it. The entire trillion-dollar AI industry is built on autocomplete. Yes, really. Your phone has been doing a much worse version of this for years \u2014 that is why you still send messages about going to the ducking store. But when you scale this up to billions of parameters trained on trillions of words, something remarkable happens. Writing, coding, reasoning \u2014 they all emerge. If people remember one thing from today, it should be: it predicts the next word.",
  },

  { section: "01 \u00b7 What Is an LLM?", color: C.blue,
    title: "The prediction machine",
    content: () => <PredictionLoop />,
    notes: "Walk through the diagram. You feed it some text. It outputs a probability for every possible next word. Paris gets 42 percent, Lyon gets 8 percent. It picks one, appends it, and loops. One word at a time. Here is the wild part: it never plans ahead. It commits to each word before thinking about the next. Imagine writing an essay but you can only see one word at a time through a keyhole. That is what these models do \u2014 and somehow they still write better emails than most of us.",
  },

  { section: "01 \u00b7 What Is an LLM?", color: C.blue,
    title: "Knowledge as a side effect",
    content: () => <EmergenceViz />,
    notes: "This is where it gets slightly mind-bending. To predict medical text, you need medical knowledge. To predict code, you need programming logic. Nobody sat down and taught it medicine. It just figured it out because knowing medicine helps you finish medical sentences. It learned everything useful as a side effect of being really good at finishing sentences. Kind of like how you accidentally memorised every word to a song you claim to hate. It was trained on text about essentially every topic humans have written about, so it absorbed knowledge about essentially every topic too.",
  },
  ...
  { section: "01 \u00b7 What Is an LLM?", color: C.blue,
    ...
    notes: "Both the brilliance and the errors come from the exact same root cause. It produces text that sounds right \u2014 because that is literally all it was trained to do. When it knows the topic, this is genuinely impressive. When it does not, it becomes the world\u2019s most confident liar. It will cite a research paper that does not exist, by an author who does not exist, published in a journal that does not exist \u2014 and it will do it with the calm authority of a professor who has read it three times. This is not a bug. It is prediction doing exactly what prediction does. It predicts what the right answer would sound like.",
  },
  ...
  { section: "02 \u00b7 How It Was Made", color: C.purple,
    ...
    notes: "Think of building an LLM like cutting a diamond. Phase one \u2014 raw: you feed it the entire internet. Everything from Shakespeare to Reddit arguments about whether a hot dog is a sandwich. World knowledge emerges as a side effect. This costs over a billion dollars. Yes, billion with a B. Phase two \u2014 cut: fine-tune it so it actually follows instructions instead of just continuing your sentence into a random Wikipedia article. Phase three \u2014 polish: humans rate thousands of responses so the model learns what being helpful actually looks like. This is where it transforms from a weird text-completion engine into the assistant you talk to. Only about five companies can afford to start from raw.",
  },
  ...
  { section: "03 \u00b7 From Brain to Body", color: C.red,
    content: () => (
      <div style={{ textAlign: "center", maxWidth: 520, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: "1rem", color: C.body, maxWidth: 480, margin: "0 auto", lineHeight: 1.6 }}>
          Imagine the smartest person who ever lived.
        </p>
        <p style={{ fontSize: "0.95rem", color: C.muted, maxWidth: 440, margin: "0.75rem auto 0", lineHeight: 1.6 }}>
          Now seal them in a room with no phone, no internet,<br/>no arms, and the memory of a goldfish.
        </p>
        <p style={{ fontSize: "0.95rem", color: C.body, marginTop: "0.75rem" }}>
          That is an LLM without tools.
        </p>
        <p style={{ fontSize: "1rem", color: C.body, fontStyle: "italic", marginTop: "1rem" }}>
          To be useful, a brain needs a body.
        </p>
      </div>
    ),
    notes: "The analogy is deliberately silly but accurate. The raw LLM can only do text in, text out. It has no tools, no memory, no way to interact with the world. When ChatGPT searches the web or writes code, that is not the model itself \u2014 it is an agent framework wrapping the LLM and giving it hands. The model provides the intelligence. The framework provides the limbs. Without the framework, Einstein is just stuck in a box, monologuing brilliantly to nobody.",
  },
  ...
  { section: "04 \u00b7 The Input Is Everything", color: C.indigo,
    ...
    notes: "This is the awkward truth. It is like hiring a world-class consultant \u2014 someone who has read every textbook, every codebase, every business report ever written \u2014 and asking them to draw a funny picture of a cat driving a car. You absolutely can. The cat picture might even be good. But maybe also ask it to help you redesign that process that has been bugging you for two years, or draft that technical spec you have been putting off. Same hourly rate either way. The right column shows where AI genuinely saves hours and produces better output than most people could alone.",
  },
  ...
  { section: "04 \u00b7 The Input Is Everything", color: C.indigo,
    ...
    notes: "Everyone in this room has done at least one of these. I certainly have. Make it better \u2014 that is like going to a restaurant and saying bring me food. Write something about AI \u2014 the model gets zero constraints so it writes the blandest thing possible. Fix this \u2014 my personal favourite. Just 500 lines of code and the phrase it does not work. The model is smart but it is not psychic. Be more creative \u2014 creativity without direction is just randomness with confidence. The fix for all of these: be specific.",
  },
  ...
  { section: "", color: C.blue,
    ...
    notes: "Three things to take away. First: it predicts text. That one fact explains everything \u2014 both why it writes beautiful prose and why it will confidently tell you the Golden Gate Bridge is in London. Second: what comes out depends on what you put in. Treat it like a very smart but very literal colleague who has never met you before. Third: the tools are here right now. The best way to learn is to use them. Start with something you know well so you can judge the output \u2014 and please, do not paste your passwords into the chat.",
  },
];
